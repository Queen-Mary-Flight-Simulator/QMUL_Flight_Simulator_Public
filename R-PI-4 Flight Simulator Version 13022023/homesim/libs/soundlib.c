...
static ALuint soundBuffers[MAX_SOUND_BUFFERS];
static ALuint soundSources[MAX_SOUND_SOURCES];
static float  soundDefaultGains[MAX_SOUND_SOURCES];

static bool         OldStallWarning;
static bool         OldFireWarning;
static bool         OldStickWarning;
static bool         OldMiddleMarker;
static bool         OldOuterMarker;
static bool         OldGearWarning;
static bool         OldConfigWarning;
static bool         OldAirconditioning;
static bool         OldElectricalNoise;
static bool         OldGearMotor;
static bool         OldSoundOn;
static unsigned int OldEngineType;  

static PulseRecord  GearWarningRecord;
static PulseRecord  ConfigWarningRecord;
static PulseRecord  OuterMarkerRecord;
static PulseRecord  MiddleMarkerRecord;
...
static bool         MorseEnabled;      /* Morse string enabled  TRUE */
static char         MorseSymbol;       /* current morse charcater */
static unsigned int MorseDigit;        /* current symbol in morse string */
static char         MorseString[5];
static bool         MorseSound;        /* sound on  TRUE */
static unsigned int MorseTimeout;      /* time to go to end of pulse in 1/50 sec */
static unsigned int MorsePulse;        /* pulse number of pulse train */
...
{
    int   m = n / sizeof(float);  /* bytes -> floats */
    int   x2;
    float r = 0.0;
    
	for (x2 = m - 1; x2 >= 1; x2 -= 1)
    {
        int x1 = x2 - 1;
        if (v >= x[x1])
        {
