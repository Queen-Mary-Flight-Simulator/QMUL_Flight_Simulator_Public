/*
    ...
    Modified for udplib DJA Feb 2020
...
int main (int argc, char* argv[])
{
    ...
    PickHandler* mouseEventHandler = new PickHandler(1.0);
    ...
    std::string screenCaptureFilename = "screenshot.png";
...
/* --------------------------------------------------------------------- */
double Bearing(double Lat1, double Long1, double Lat2, double Long2) /* bearing of (lat1, long1) from (lat2, long2) */
{
  double x, y;
  double ax, ay;
  double dLat;
...
//        q = normalise(rads((double) (IGPkt.RunwayQDM)) + (double) (IGPkt.MagneticVariation) );
	...
	if (CameraPosition > 0) 
	{
		...
		//	Targets[0].yaw = normalise(Targets[0].yaw);
		//}
		else if (CameraPosition == 4) // above
		{
			Targets[0].z = 1000.0;
			Targets[0].pitch = -M_PI_2;
			Targets[0].yaw = 0.0;
			Targets[0].yaw = 0.0;
		}
    }
	...
	if ( IGPkt.TimeOfDay != OldTimeOfDay )
	{
		SetTime( (float) (IGPkt.TimeOfDay) / 60.0, float(a) );
		OldTimeOfDay = IGPkt.TimeOfDay;
	}
