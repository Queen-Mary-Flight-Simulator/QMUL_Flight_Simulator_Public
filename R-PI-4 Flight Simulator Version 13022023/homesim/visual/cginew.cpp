...
#if OPENSCENEGRAPH_SOVERSION > 100 //If osg > 3.2.1 //OSG
  #include <osg/Camera>
#else
...
#pragma pack(push,2)
#endif
#include "network_protocol/iodefn.h"
#include "network_protocol/aerodefn.h"
#include "network_protocol/navdefn.h"
#include "network_protocol/iosdefn.h"
#ifdef WIN32
#pragma pack(pop)
#endif
#include "hud.h"
...
*/
#define RUNTIME_DIR "C:/wkorek/env/"
#define SCENERY_FOLDER RUNTIME_DIR "scenery/"
#define MODELS_FOLDER RUNTIME_DIR "models/"

/*Manual channel defines*/
...
*/
/*Automatic defines (from Makefile)*/
/*#if defined LEFT_CHANNEL
  #define ChannelOffset 60.0f //#define ChannelOffset 43.45f
#elif defined RIGHT_CHANNEL
  #define ChannelOffset -60.0f //#define ChannelOffset -43.45f
#else
...
class HUD : public osg::Drawable
{
    public:
        ...
        virtual void drawImplementation(osg::RenderInfo&) const
        {
			...
			/*Manual channel define*/
			drawHUD();
			/*Makefile channel define*/
          /*#ifdef CENTRE_CHANNEL
            drawHUD();
          #endif*/
        }
...
#if OPENSCENEGRAPH_SOVERSION > 100 //If osg > 3.2.1
...
{
    int     frameNum = 0;
    int     s, i, retval; // s - socket fd; retval - return value from socket_get_data in main while loop
    double  x, y, z, h, p, r;
    ...
    osg::ref_ptr<osg::Node> SkyBoxNode = osgDB::readNodeFile(MODELS_FOLDER "skyb.ac");
    if (!SkyBoxNode)
    {
    ...
    //viewer.setUpViewInWindow(0,0,640,480); //W.K.
    ...
    // So OSG aspect ratio of wants tan(fovx/2)/tan(fovy/2) = 1.55877
    ...
    while (!viewer.done())
    {
        ...
        //vecPosAircraft.set(0,0,2);
        //vecAttAircraft.set(0,0,0);
...
int SelectDatabase ( char *argv[] )
{
    if ( !strcmp ( argv[1], "bristol" ) )
    {
        ...
        pathList.push_back(SCENERY_FOLDER "databases/long_bristol/flt_files");
        pathList.push_back(SCENERY_FOLDER "databases/long_bristol/models");
        pathList.push_back(SCENERY_FOLDER "databases/long_bristol/texture");
    }
	else if ( !strcmp ( argv[1], "sheffield" ) )
    {
        ...
        pathList.push_back(SCENERY_FOLDER "databases/sheffield_output/standard_openflight/geometry");
        pathList.push_back(SCENERY_FOLDER "databases/long_bristol/models");
        pathList.push_back(SCENERY_FOLDER "databases/sheffield_output/standard_openflight/texture/used_textures");
	}
    ...
    else if ( !strcmp ( argv[1], "bristol_carrier" ) )
    {
	    ...
	    setname("long_bristol.flt");
	    pathList.push_back(SCENERY_FOLDER "databases/Jake/Long_Bristol/FLT_files");
    }
    else if ( !strcmp ( argv[1], "hong_kong" ) )
    {
        RunwayX  =   -18552.0; //RunwayX  =   -21752.0;
        RunwayY  =    -3003.0; //RunwayY  =    -2238.0;
        RunwayZ  =        0.0;
        RunwayRotation =  1.815142422;		/* 104 degrees */ ////RunwayRotation =  1.815142422;		/* 104 degrees */

		RunwayQDM =      250.5;             /* 25L QDM True */
        setname("hk_sgi_demo.flt");
        pathList.push_back(SCENERY_FOLDER "databases/hong_kong");
    }
    else if ( !strcmp ( argv[1], "monterey" ) )
    {
        ...
        pathList.push_back(SCENERY_FOLDER "databases/long_monterey/flt_files");
        pathList.push_back(SCENERY_FOLDER "databases/long_monterey/models");
        pathList.push_back(SCENERY_FOLDER "databases/long_monterey/textures");
    }
	...
	//pathList.push_back(SCENERY_FOLDER "databases/Performer/data");
	pathList.push_back(SCENERY_FOLDER "databases/cranfly2002/flt_files");
    }
    ...
    else if ( !strcmp ( argv[1], "bayport" ) )
    {
        ...
        setname("bayport.flt");
        pathList.push_back(SCENERY_FOLDER "databases/bayport/flt_files");
        pathList.push_back(SCENERY_FOLDER "databases/bayport/models");
        pathList.push_back(SCENERY_FOLDER "databases/bayport/textures");
    }
    else if ( !strcmp ( argv[1], "fresno" ) )
    {
        ...
        setname("fresno.flt");
        pathList.push_back(SCENERY_FOLDER "databases/fresno/flt_files");
        pathList.push_back(SCENERY_FOLDER "databases/fresno/models");
        pathList.push_back(SCENERY_FOLDER "databases/fresno/textures");
    }
	...
	pathList.push_back(SCENERY_FOLDER "databases/heathrow/flt_files");
    }
    else if ( !strcmp ( argv[1], "london" ) )
    {
	    ...
	    pathList.push_back(SCENERY_FOLDER "databases/london");
        pathList.push_back(SCENERY_FOLDER "databases/london/textures");
	}
	else if ( !strcmp ( argv[1], "manchester" ) ) // from EFS code
    {
        RunwayX  =         0.0;
        RunwayY  =         -1091.0;
        RunwayZ  =         -7.26;
        ...
        pathList.push_back(SCENERY_FOLDER "databases/manchester");
    }
    else
    {
        return -1;
    ...
    pathList.push_back(SCENERY_FOLDER "multigen");
    pathList.push_back(SCENERY_FOLDER "multigen/models");
    pathList.push_back(SCENERY_FOLDER "multigen/texture");
        ...
        switch (IosCmd)
        {
            ...
            case IosDefn_LoadTargetFile:
                ...
                if (TargetLoaded)
                {
                    ...
                    sunLight->removeChild(TargetXForm2.get());
                    TargetXForm2->removeChild(TargetNode2.get());
                }


                TargetNode = osgDB::readNodeFile(Filename);
                TargetNode2 = osgDB::readNodeFile(Filename);
                ...
                TargetXForm2->addChild(TargetNode2.get());
                TargetXForm->addChild(TargetNode.get());
                sunLight->addChild(TargetXForm.get());   // already attached to root
                sunLight->addChild(TargetXForm2.get());   // already attached to root
...
int socket_init(void)
{
    ...
    //addr_info.sin_addr.s_addr = inet_addr("192.168.1.5");
    ...
    permission = 1;
    if (setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (const char *) &permission, sizeof(permission)) < 0)
    {
            ...
            case 3:
                ...
                else
                {
					...
					*/
                    ...
                    T2Pitch = AeroPkt.ExtTargets_Pitch[0];
                    T2Roll = AeroPkt.ExtTargets_Roll[0];
                    T2Yaw = AeroPkt.ExtTargets_Yaw[0];
                    ...
                    if (NavPkt.CurrentRunway > 0)
                    {
                        ...
                        d = Distance(AeroPkt.ExtTargets_Latitude[0], AeroPkt.ExtTargets_Longitude[0],
                                     (double) NavPkt.RunwayLatitude, (double) NavPkt.RunwayLongitude);
                        b = Bearing(AeroPkt.ExtTargets_Latitude[0], AeroPkt.ExtTargets_Longitude[0],
                                    (double) NavPkt.RunwayLatitude, (double) NavPkt.RunwayLongitude);
                        T2x = -d * sin(b);
                        T2y = -d * cos(b);
                        T2z = -AeroPkt.ExtTargets_Pz[0] + NavPkt.GroundLevel;
                    ...
                    Targets[2].y = T2y;
                    Targets[2].z = T2z;
                    Targets[2].pitch = T2Pitch;
                    Targets[2].roll = T2Roll;
                    Targets[2].yaw = T2Yaw;
                ...
                px = Targets[2].x * cosa + Targets[2].y * sina + RunwayX;
                py = Targets[2].y * cosa - Targets[2].x * sina + RunwayY;
                pz = Targets[2].z + RunwayZ;
                p = degrees(Targets[2].pitch);
                h = degrees(RunwayRotation - Targets[2].yaw + q);
                r = degrees(Targets[2].roll);

                vecPosTarget2.set(px, py, pz);
                vecAttTarget2.set(h, p, r);
                ...
                if (TargetLoaded)
                {
                    ...
                    TargetXForm2->setAttitude( osg::Quat(osg::DegreesToRadians(vecAttTarget2.z()), osg::Vec3(0,1,0),  // roll
                                              osg::DegreesToRadians(vecAttTarget2.y()), osg::Vec3(1,0,0),  // pitch
                                              osg::DegreesToRadians(vecAttTarget2.x()), osg::Vec3(0,0,1))); // heading

                    TargetXForm2->setPosition( vecPosTarget2 );
                }
