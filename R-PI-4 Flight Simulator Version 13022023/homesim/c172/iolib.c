...
int convert16(short unsigned int x);
...
/* ---------------------------------------------------- */    
int oldconvert16(short unsigned int x)
{
    int a = x & 0xff;
    int b = (x >> 8) & 0xff;
    int r = (b << 8) | a;
...
/* ---------------------------------------------------- */    
int convert16(short unsigned int x)  /* sign extend to 32 bits */
{
    int r = (int) x;
...
float IOLib_GetEngineLever(unsigned int LeverNumber)
{
	...
	if (LeverNumber < 2)
	{
        e = convert16(AeroLink_IOPkt1.AnalogueData[5]);  /* port engine(s) */
	}
	else
	{
        e = convert16(AeroLink_IOPkt1.AnalogueData[4]);  /* starboard engine(s) */
	}
...
{
    int e = convert16(AeroLink_IOPkt1.AnalogueData[1]);
...
{
    int a = convert16(AeroLink_IOPkt1.AnalogueData[0]);
...
{
    int r = convert16(AeroLink_IOPkt1.AnalogueData[2]);
...
{
	float                               t = -(float) convert16(AeroLink_IOPkt1.AnalogueData[3]) / 32767.0;
...
{
    int t = convert16(AeroLink_IOPkt1.AnalogueData[8]);
...
{
    int t = convert16(AeroLink_IOPkt1.AnalogueData[9]);
...
{
    int b = convert16(AeroLink_IOPkt1.AnalogueData[7]);
...
{
    int b = convert16(AeroLink_IOPkt1.AnalogueData[6]);
    return (float) (32767 - b) / 65536.0;
}
