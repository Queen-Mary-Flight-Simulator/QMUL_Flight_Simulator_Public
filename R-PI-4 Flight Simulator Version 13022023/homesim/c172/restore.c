...
void PrintRestoreFile(IosDefn_RestoreVectorRecord a)
{
    ...
    printf("FCU_ALT_HOLD: %s\n", KnobPosition(a.FCU_ALTKnob));
    printf("FCU_HDG_HOLD: %s\n", KnobPosition(a.FCU_HDGKnob));
    printf("FCU_SPD_HOLD: %s\n", KnobPosition(a.FCU_SPDKnob));
    printf("FCU_VS_HOLD: %s\n\n", KnobPosition(a.FCU_VSKnob));
...
char *KnobPosition(NavDefn_FCUKnob k)
{
	...
	else if (k == NavDefn_Middle)
	{
	    return "Middle";
	}
	else if (k == NavDefn_Pulled)
	{
	    return "Pulled";
	}
	else
...
void ModifyAndWriteRestoreFile(IosDefn_RestoreVectorRecord *a)
{
	...
	// M1 J29 Centre (Nr Chesterfield)
	a->Latitude = Radians(53.2833);
	a->Longitude = Radians(-2.4);
	
	a->SavedRadios[0].NavILS.Active = 10950;
	a->SavedRadios[0].NavVOR.Active = 11355;
	a->Pitch = 0.0;
	a->Roll = 0.0;
