...
void Map_CompassRose(int x, int y)
{
    ...
    dm = -Maths_Rads(MagneticVariation);
    ...
    {
	    Glib_PushMatrix();
        Glib_Rotate((float) (360 - a - MagneticVariation));
...
void DrawRunway(int x1, int y1, NavLib_RunwayRecord r)
{
    ...
    q = r.Qdm + Maths_Rads(MagneticVariation);
    q = Maths_Normalise(q);
    Len = (float)r.Length;
...
void DrawBeacon(int x, int y, NavLib_BeaconRecord n)
{
    ...
    char              Ident[6];
    float             qdm = n.Qdm + Maths_Rads(MagneticVariation);
...
void Map_ShowAircraftTrack(int New) // bool param
{
    ...
    if (AircraftTrackList->DisplayedPoints < AircraftTrackList->TrackLength)
    {
        ...
        do
        {
            ...
            {
                Map_GlobeToScreen(AircraftTrackList->xPoints[PrevTrackPoint(AircraftTrackList->NextDisplayedPoint)],
                                  AircraftTrackList->yPoints[PrevTrackPoint(AircraftTrackList->NextDisplayedPoint)],
                                  &sx1, &sy1);
