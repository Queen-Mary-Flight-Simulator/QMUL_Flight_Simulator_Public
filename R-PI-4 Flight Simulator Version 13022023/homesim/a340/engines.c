...
//printf("Thrust=%f EPR=%f MACH=%f\n", Engines_Engines[0].Thrust / 4.4482, Engines_Engines[0].Epr, Model_MachNumber);
}
...
static void Engine(float ThrottlePosition, float ReversePosition, unsigned int EngineNumber, bool Failed)
{
    ...
    {
        bool  FuelSwitch = (IOLib_GetFuelSwitch(EngineNumber) == IODefn_On);
        bool  StarterSwitch = (IOLib_GetStarterSwitch(EngineNumber) == IODefn_On);
        bool  StartSelected = FuelSwitch && StarterSwitch;
        
        float h = Maths_Feet(-Model_Pz);  /* altitude (ft) */
        float tempF = (Weather_Temperature(-Model_Pz) - 273.15) * 9.0f / 5.0f + 32.0f;  /* degK -> degF */
        ...
        //printf("e=%f ThrottlePosition=%f lever_angle=%f tempF=%f\n", e, ThrottlePosition, lever_angle, tempF); // ***	
        ...
        if (!DEMO_MODE) /* comment out next 14 lines for demo mode */
        {
