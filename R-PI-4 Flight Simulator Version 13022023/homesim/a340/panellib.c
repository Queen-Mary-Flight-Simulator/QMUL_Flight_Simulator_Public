...
void Knob(int x, int y, int sx, int sy, float angle);
void KnobInner(int x, int y, int sx, int sy, float angle);
void KnobBlue(int x, int y, int sx, int sy, float angle);
                         ...
                         float VS, unsigned char hPa)
{
    ...
    Knob(62, 67, 38, 38, Baro);

    Knob(383, 83, 38, 38, Speed);
    KnobBlue(453, 83, 38, 38, Hdg);
    Knob(623, 83, 38, 38, Alt);
    Knob(714, 83, 38, 38, VS);
}
                           ...
                           float InnerKnobPosition, float OuterKnobPosition)
{
    /* Radio Panel */
    Glib_LoadIdentity();
    Glib_Translate((float) NFD_RadioX, (float) NFD_RadioY);
