 ...
 * cgi.c
 * 
 * This plugin registers a new view in XPlane-11 based on data in the UDP packet (igpkt) sent by the PC
 * The XPlane camera is commanded to provide a view using the aircraft position and attitude by:
 * 
...
/* ------------------------------------------------------------------ */
/* N.B. Only initialise UDP library after F8 pressed to avoid any packets being queued */
void MyHotKeyCallback(void * inRefcon)
{
    /* we control the camera until the view changes. */
    ...
    QueryPerformanceFrequency((void *) &freq);  /* initialise timers */
...
QueryPerformanceCounter((void *) &Timer3);  // ***
    ...
    /* N.B. XPlane crashes if the aircraft or target is positoned at (0, 0)
	   Latitude and longitude are in degrees, altitude is in metres above MSL.
       Note, XPlane local coords are moved to keep the aircraft near a reference origin
       so local x,y,z can be floats. We have no control over the local origin, so must
       provide XPlane with lat/long and it will determine the correct x,y,z. */
...
int UDP_Recv(void)
{
    ...
    do
    {
        ...
        if (p == 1)  /* I/O RPi 1 */
        {
            ...
            if ((Timer2 - Timer1) > 220000)  /* frame > 22 ms? */
            {
                do
                {
