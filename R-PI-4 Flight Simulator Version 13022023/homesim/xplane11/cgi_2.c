...
XPLMDataRef  Lat0Ref          = NULL;
XPLMDataRef  Long0Ref         = NULL;
                        ...
                        char *		outDesc)
{
	...
	Lat0Ref          = GetRef("sim/flightmodel/position/latitude");
    Long0Ref         = GetRef("sim/flightmodel/position/longitude");
                     ...
                     void                 *inRefcon)
{
	...
	if (CameraPosition == 0)  /* place the camera in the aircraft cockpit */
	{
	  ...
	  //outCameraPosition->y       = (float) (AircraftY - IGPkt.Ez + 12.5);   /* up component (12.5 Manchester database adjustment) */
		outCameraPosition->y       = (float) (AircraftY - IGPkt.Ez - 0.75);   /* up component (0.75 Heathrow database adjustment) */
...
/*  open sockets from RPi 192.168.1.1 and IG 192.168.1.2 for IG 192.168.1.8 */    
	...
	printf("Lat0=%f Long0=%f\n", XPLMGetDataf(Lat0Ref), XPLMGetDataf(Long0Ref)); // ***
