...
#define DEG80                     (80.0 / Maths_ONERAD)
...
typedef struct 
{
    ...
    float             EndLatitude;     /* rads */
    float             EndLongitude;    /* rads */
...
NavLib_RunwayInfo     LocalRunways[20];
unsigned int          nLocalRunways;

unsigned int          LineNumber;
FILE                  *FileStream;
unsigned int          HashTable[HashTableSize];

static unsigned int   NearestILSRunway(float latitude, float longitude, int freq, unsigned int runway);
...
void                  RunwayQDM(char str[], unsigned int runway);
static unsigned int   Hash(unsigned int f);
static void           InitialiseHashTable();
static bool           Reciprocal(unsigned int runway1, unsigned int runway2);
static bool           Runways_Read;

/* --------------------------------------------------- */ 
unsigned int NavLib_NearestRunway(float latitude, float longitude, int freq)  /* latitude rads, longitude rads */
{
    unsigned int i;
    float        MinDistanceILS    = 1000000.0;
    float        MinDistanceInView = 1000000.0;
    float        MinDistance       = 1000000.0;
    unsigned int NearestILS        = 0;
    unsigned int NearestInView     = 0;
    unsigned int Nearest           = 0;
      
    for (i=1; i<=NavLib_NumberOfRunways; i+=1)
    {
        float d = NavLib_Distance(latitude, longitude, NavLib_Runways[i].RunwayEndLatitude, NavLib_Runways[i].RunwayEndLongitude);
        float b = NavLib_Bearing(latitude, longitude, NavLib_Runways[i].RunwayEndLatitude, NavLib_Runways[i].RunwayEndLongitude);
        float a = b - NavLib_Runways[i].Qdm;
		
        Maths_Normalise(&a);

        if (NavLib_Runways[i].Frequency == freq && d < MinDistanceILS && d < 50000 && fabs(a) < DEG80)
        {
            MinDistanceILS = d;
            NearestILS = i;
        }
        
        if (d < 15000.0 && d < MinDistanceInView && fabs(a) < DEG80)
        {
            MinDistanceInView = d;
            NearestInView = i;
        }
    ...
    else if (NearestInView > 0)
    {
        return NearestInView;
    }
    else
    {
        return Nearest;
...
static unsigned int NearestILSRunway(float latitude, float longitude, int freq, unsigned int runway)
{
    ...
    for (i=1; i<=NavLib_NumberOfRunways; i+=1)
    {
        float d = NavLib_Distance(latitude, longitude, NavLib_Runways[i].RunwayLatitude, NavLib_Runways[i].RunwayLongitude);
        
        if (NavLib_Runways[i].Frequency == freq && NavLib_Runways[i].Runway == runway && d < dmin)
        {
            dmin = d;
            b = i;
        }
...
/*
    ...
    {
        RunwayQDM(str, NavLib_Beacons[i].Runway);
		printf("%d: id=%s lat=%f long=%f freq=%d range=%d alt=%d qdm=%f %s %s\n", i,
    ...
    for (i=1; i<=NavLib_NumberOfRunways; i+=1)
    {
	    RunwayQDM(str, NavLib_Runways[i].Runway);
        printf("%d %s: lat=%f long=%f elevation=%d len=%d width=%d qdm=%f freq=%d %s\n", i, 
...
void NavLib_ReadBeacons(float minlat, float maxlat, float minlong, float maxlong)  /* args in degrees */
{
    ...
    while (1)
    {
        ...
        if (latitude > minlat && latitude < maxlat && longitude > minlong && longitude < maxlong)
        {
            ...
            if (t & NavLib_ILS)
            {
                unsigned int ils = NearestILSRunway(Maths_Rads(latitude), Maths_Rads(longitude), NavLib_Beacons[NavLib_NumberOfBeacons].Frequency, NavLib_Beacons[NavLib_NumberOfBeacons].Runway);
                ...
                if (ils > 0)
                {
                    ...
                    //printf("ILS modified: %s %f %f\n", NavLib_Beacons[NavLib_NumberOfBeacons].Ident, rlat, rlong);
                }
            }
        }
    }
        ...
        if (latitude > minlat && latitude < maxlat && longitude > minlong && longitude < maxlong)
        {
            ...
            while (1)
            {
                ...
                if (str[0] == 'R')
                {
                    ...
                    FileIO_ReadInt();  /* skip actual runway QDM */
            ...
            for (i=1; i<=nLocalRunways; i+=1)
            {
                ...
                for (j=1; j<=nLocalRunways; j+=1)
                {
                    if (i != j)
                    {
                        if (Reciprocal(LocalRunways[i].Runway, LocalRunways[j].Runway))
                        {
							LocalRunways[i].Rqdm = NavLib_Bearing(LocalRunways[i].Latitude, LocalRunways[i].Longitude, LocalRunways[j].Latitude, LocalRunways[j].Longitude);
							LocalRunways[i].EndLatitude = LocalRunways[j].Latitude;
							LocalRunways[i].EndLongitude = LocalRunways[j].Longitude;
                            break;
                        } 
                    }
                }
            ...
            for (i=1; i<=nLocalRunways; i+=1)
            {
                ...
                NavLib_Runways[NavLib_NumberOfRunways].RunwayLatitude     = LocalRunways[i].Latitude;
                NavLib_Runways[NavLib_NumberOfRunways].RunwayLongitude    = LocalRunways[i].Longitude;
                NavLib_Runways[NavLib_NumberOfRunways].RunwayEndLatitude  = LocalRunways[i].EndLatitude;
                NavLib_Runways[NavLib_NumberOfRunways].RunwayEndLongitude = LocalRunways[i].EndLongitude;
                NavLib_Runways[NavLib_NumberOfRunways].Frequency          = LocalRunways[i].Frequency;
                NavLib_Runways[NavLib_NumberOfRunways].Qdm                = LocalRunways[i].Rqdm;
                NavLib_Runways[NavLib_NumberOfRunways].Length             = LocalRunways[i].Length;
                NavLib_Runways[NavLib_NumberOfRunways].Width              = LocalRunways[i].Width;
                NavLib_Runways[NavLib_NumberOfRunways].Elevation          = LocalRunways[i].Elevation;
                NavLib_Runways[NavLib_NumberOfRunways].Runway             = LocalRunways[i].Runway;
...
/*				
                printf("%s %s: Lat=%f Long=%f Freq=%d QDM=%f Length=%d Width=%d Elevation=%d\n", 
                    ...
                    Maths_Degrees(NavLib_Runways[NavLib_NumberOfRunways].Qdm),
...
static bool Reciprocal(unsigned int runway1, unsigned int runway2)
{
    unsigned int r1 = runway1 % 100;
    unsigned int r2 = runway2 % 100;
	...
	if (runway1 < 100 && runway2 < 100)
	{
	    return r1 == r2;
	}
	
	if (runway1 < 200 && runway2 > 200)
	{
	    return r1 == r2;
	}
