...
bool AeroLink_RespondToIos()
{
    ...
    while (1)
    {
        switch (GetWord())
        {
            ...
            case 193:   // added for QMU version 1 
			    Systems_FlapSetting = (int) GetReal();
				break;
				
			case 194:
			{   int p = GetInt();
			    Systems_GearSelector = (p == 0) ? 1 : 2;   // added for QMU version 1
printf("Gear: selector=%d\n", p); // *** 
				break;
			}
...
void AeroLink_FormPacket()
{
    ...
    AeroLink_AeroPkt.APSpeedMode        = FCS_APSpeedMode;
...
/* ------------------------------------------------- */
    ...
    AeroLink_IGPkt.Stopping   = AeroLink_Stopping;
