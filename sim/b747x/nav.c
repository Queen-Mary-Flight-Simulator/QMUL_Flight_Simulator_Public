...
#define PI            Maths_PI
#define ONERAD        Maths_ONERAD
#define TWOPI         Maths_TWOPI
#define PIBY2         Maths_PIBY2
...
float Nav_DmeDistance, Nav_GroundSpeed;
...
void dNormalise(double *x);
float GetTAS(float IAS, float Z);

/* ------------------------------------------------------ */
float GetTAS(float IAS, float Z)   /* nb. IAS (m/s), Z +ve altitude (m) */
{
...
void Nav_UpdateNav()
{
    ...
    if (FCU_ModeSelector == NavDefn_ModeILS) 
    {
        ...
        if (Nav_ILS1.BeaconStatus) 
		{
            ...
            Nav_GroundSpeed = Nav_ILS1.Gspeed;
        } else 
		{
	...
	else 
	{
        ...
        if (Nav_VOR1.BeaconStatus) 
		{
            ...
            Nav_GroundSpeed = Nav_VOR1.Gspeed;
        } 
		else 
		{
            Nav_DmeDistance = -10000.0;
	...
	{
        Nav_Rmi_Dir1 = Nav_ADF1.RmiPointer;
    } 
	else if (FCU_NavSwitch1 == NavDefn_NavVOR) 
	{
        Nav_Rmi_Dir1 = Nav_VOR1.RmiPointer;
    } 
	else 
	{
        Nav_Rmi_Dir1 = Maths_PIBY2;
    }
    if (FCU_NavSwitch2 == NavDefn_NavVOR) 
	{
        Nav_Rmi_Dir2 = Nav_VOR2.RmiPointer;
    } 
	else if (FCU_NavSwitch2 == NavDefn_NavADF) 
	{
        Nav_Rmi_Dir2 = Nav_ADF2.RmiPointer;
    } 
	else 
	{
        Nav_Rmi_Dir2 = Maths_PIBY2;
    }

    Nav_CurrentRunway = NavLib_NearestRunway(NavLink_AeroPkt.Latitude, NavLink_AeroPkt.Longitude, NavLib_Beacons[Nav_ILS1.SelectedBeacon].Frequency);
...
static void NavCalculations(Nav_NavRecord *Nav, float Obs, bool Operational)
{
    ...
    //printf("SelectedBeacon : %d\n", Nav->SelectedBeacon); fflush(stdout); // ***
    ...
    if (OK) 
	{
        ...
        if (dslant <= MaxRange) 
		{
            ...
            Maths_Normalise(&Nav->LocaliserError);
            Nav->RmiPointer = Nav->BearingToStation - (float) NavLink_AeroPkt.Yaw;
            Nav->Gspeed = sqrt((float) NavLink_AeroPkt.Vn * (float) NavLink_AeroPkt.Vn +
