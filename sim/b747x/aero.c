...
unsigned int SplineNumber(SplineData s, float x);

/* ------------------------------------------------------------------ */
float Lookup1(SplineData s, unsigned int n, float x)
{
    return splint(s.Lines[n].xa, s.Lines[n].ya, s.Lines[n].y2, s.Lines[n].np, x);
}

/* ------------------------------------------------------------------ */
float Lookup2(SplineData s, float x, float z)
{
    float y1;
    float y2;
    float dz;
    ...
    cn = SplineNumber(s, z);
    dz = (s.Maxz - s.Minz) / (float) (s.nLines - 1);  /* assumption: z lines are equally spaced */
    z0 = s.Minz + cn * dz;
    y1 = Lookup1(s, cn, x);
    y2 = Lookup1(s, cn + 1, x);
    return y1 + (z - z0) * (y2 - y1) / dz;
}

/* ------------------------------------------------------------------ */
unsigned int SplineNumber(SplineData s, float z)
{
    int cn;

    cn = (unsigned int) ((float) (s.nLines - 1) * (z - s.Minz) / (s.Maxz - s.Minz));
    if (cn > (s.nLines - 2))
    {
        cn = s.nLines - 2;
    }
