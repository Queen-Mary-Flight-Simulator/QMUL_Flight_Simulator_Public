...
void Model_FlightModel()
{
    ...
    else
    {
        ...
        Model_Alpha    = atan(Model_W / Model_U);
        Model_Beta     = atan(Model_V / sqrt(uw2));
    ...
    Maths_Limit(&Model_GForce, -6.0, 10.0);
    ...
    Maths_Integrate(&U_aero, Model_UDot);
    Maths_Integrate(&V_aero, Model_VDot);
    Maths_Integrate(&W_aero, Model_WDot);
    ...
    if (!AeroLink_Freezing)
    {
        ...
        Maths_Double_Integrate(&Model_Latitude, Model_dLatitude);
        Maths_Double_Integrate(&Model_Longitude, Model_dLongitude);
        Maths_Integrate(&Model_Pz, Model_Vd);
    }
    ...
    Model_Rmt = 0.5 * Weather_Rho * Vc2 * Aero_s * Aero_b *
                (Model_ClBeta * Model_Beta + Model_ClDa * Da + Model_ClDr * Dr) + 0.25 *
                Weather_Rho * Model_Vc * Aero_s * Aero_b * Aero_b *
    ...
    Maths_Integrate(&Model_P, Model_PDot);
    Maths_Integrate(&Model_Q, Model_QDot);
    Maths_Integrate(&Model_R, Model_RDot);

    Maths_Limit(&Model_P, -Maths_PI, Maths_PI);
    Maths_Normalise(&Model_P);
    Maths_Normalise(&Model_Q);
    Maths_Normalise(&Model_R);
...
/*  if (Model_U < 1.0) 
    ...
    Maths_Integrate(&Model_e0, e0Dot);
    Maths_Integrate(&Model_e1, e1Dot);
    Maths_Integrate(&Model_e2, e2Dot);
    Maths_Integrate(&Model_e3, e3Dot);
    Model_SetDCM();
    SetEyePosition();
    t = -Model_A31;
    Maths_Limit(&t, -1.0, 1.0);
    Model_Pitch = asin(t);
