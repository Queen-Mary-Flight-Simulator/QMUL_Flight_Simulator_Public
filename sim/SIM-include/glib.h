...
extern void Glib_DrawTexQuad(float X, float Y, float XS, float YS, float rot, int anchor, GLuint texobj);
extern void Glib_DrawTexQuadUV(float X, float Y, float XS, float YS, float rot, int anchor, GLuint texobj, float u0, float u1, float v0, float v1);
extern GLuint Glib_GenerateTexture(GLuint textureArray[], char *FileName, int textureID, GLint wrap);
