/* +------------------------------+---------------------------------+
   | Module      : weather.c      | Version : 3.1                   | 
   | Last Edit   : 27-11-2021     | Ref     : 03-01-13              |
   +------------------------------+---------------------------------+
   | Computer    : PFD                                              |
...
void Weather_WeatherModel(bool turbulence, float Pz, float U)
{
    ...
    const double g0     =   9.80665;         /* m/s^2, acceleration of gravity at 45.5425 deg lat. */
    const double r0     = 6356766.0;         /* m, Earth radius at g0 */
    const double P0     =  101325.0;         /* Pa, pressure at g0 */
    const double T0     =    288.15;         /* K, standard sea-level temperature */
    const double Gamma  =    1.40;           /* ratio of specific heats for ideal diatomic gas */
    const double GMR    = g0 * M0 / Rs;      /* gas constant */
    const double TLAPSE = -0.0065;           /* deg/m, temperature lapse*/
    const double T11    = 216.65;            /* temperature at 11,000 m */
    const double P11    = P0 * 0.2233631;    /* pressure at 11,000 m */
    ...
    {
        T = T0 + TLAPSE * H;
        P = P0 * pow(T0 / T, GMR / TLAPSE);
    }
    else
    {
        T = T11;
        P = P11 * exp(-GMR * (H - 11000.0) / T11); 		
    }

    Weather_Temperature      = (float) T;
    Weather_Pressure         = (float) P;
