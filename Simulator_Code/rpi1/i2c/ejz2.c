/* MCP4728 DAC saw-tooth test Vout A */

#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
#include <fcntl.h>
#include <unistd.h>
#include <sys/ioctl.h>
#include <linux/i2c.h>
#include <linux/i2c-dev.h>
#include <sys/time.h>

/* LEDS 8-bit reg at 0x21 */

int main()
{
    int                        i2c;
    long unsigned int          Timer1;
    long unsigned int          Timer2;
    int               pattern = 1; 
    bool                       right = false;
    struct timeval             tv;
    unsigned char              buf[3];
    struct i2c_rdwr_ioctl_data packets;
    struct i2c_msg             messages[1];

    i2c = open("/dev/i2c-1", O_RDWR);
    if (i2c < 0)
    {
        printf("unable to open i2c as a file\n");
        exit(1);
    }

    buf[0] = 0;  /* write 0 to reg 0 (set dir to output) */
    buf[1] = 0;

    messages[0].addr = 0x21;
    messages[0].flags = 0;
    messages[0].len = 2;
    messages[0].buf = buf;

    packets.msgs = messages;
    packets.nmsgs = 1;

    if (ioctl(i2c, I2C_RDWR, &packets) < 0)
    {
        printf("unable to write to dir register\n");
        exit(1);
    }

    gettimeofday(&tv, NULL);
    Timer1 = tv.tv_usec;

    while (1)
    {
        do
        {
            gettimeofday(&tv, NULL);
            Timer2 = tv.tv_usec;
            if (Timer2 < Timer1)
            {
                Timer2 += 1000000L;
            }
        } while (Timer2 <= (Timer1 + 5000L));  /* 200 iterations  every second */
        Timer1 = (Timer1 + 5000L) % 1000000L;

        buf[0] = 9;  /* write pattern to reg 9 */
        buf[1] = (unsigned char) (pattern >> 4);

        messages[0].addr = 0x21;
        messages[0].flags = 0;
        messages[0].len = 2;
        messages[0].buf = buf;

        packets.msgs = messages;
        packets.nmsgs = 1;

        if (ioctl(i2c, I2C_RDWR, &packets) < 0)
        {
            printf("unable to write to MCP23008 data register\n");
            exit(1);
        }

        buf[0] = 0x58 + 6;  /* select DAC D */
        buf[1] = (unsigned char) (pattern >> 8);
        buf[2] = (unsigned char) pattern;

        messages[0].addr = 0x60;
        messages[0].flags = 0;
        messages[0].len = 3;
        messages[0].buf = buf;

        packets.msgs = messages;
        packets.nmsgs = 1;

        if (ioctl(i2c, I2C_RDWR, &packets) < 0)
        {
            printf("unable to write to MCP4728 data registers\n");
            exit(1);
        }

        if (!right) 
        {
            pattern = pattern + 1;
            if (pattern >= 0xFFF)
            {
                right = true;
            }
        }
        else
        {
            pattern = pattern - 1;
            if (pattern <= 0)
            {
                right = false;
            }
        }
    }
    return 0;
}
