...
void Gear_GearModel()
{
    ...
    float spitch = sin(Model_Pitch);
    float cpitch = cos(Model_Pitch);
    float sroll  = sin(Model_Roll);
    float croll  = cos(Model_Roll);
    float syaw   = sin(Model_Yaw);
    float cyaw   = cos(Model_Yaw);

    float spsr   = spitch * sroll;
    float spcr   = spitch * croll;
    float cpsr   = cpitch * sroll;
    float cpcr   = cpitch * croll;
    float cpsy   = cpitch * syaw;
    float cpcy   = cpitch * cyaw;
    float crsy   = croll * syaw;
    float crcy   = croll * cyaw;

    float A11    = cpcy;             /* same as model DCM */
    float A12    = spsr * cyaw - crsy;
    float A21    = cpsy;
    float A22    = spsr * syaw + crcy;
...
/* nb. dimensions in ft */
    ...
    if (dst1 < 0.0)              /* nose gear */
    {
        ...
        fs1 = Maths_Limit(fs1, -fabs(0.6 * fng1), fabs(0.6 * fng1));
    ...
    if (dst2 < 0.0)              /* left main gear */
    {
        ...
        fs2 = Maths_Limit(fs2, -fabs(0.6 * fng2), fabs(0.6 * fng2));
        fb2 = Model_LeftBrake * 10.0 * 2.0 * 0.263 * w / g;
        fb2 = Maths_Limit(fb2, 0.0, fbmax);
    ...
    if (dst3 < 0.0)              /* right main gear */
    {
        ...
        fs3 = Maths_Limit(fs3, -fabs(0.6 * fng3), fabs(0.6 * fng3));
        fb3 = Model_RightBrake * 10.0 * 2.0 * 0.263 * w / g;
        fb3 = Maths_Limit(fb3, 0.0, fbmax);
