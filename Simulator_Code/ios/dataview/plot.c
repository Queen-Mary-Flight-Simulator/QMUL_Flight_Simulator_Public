       ...
       for (i = 1; i <= NumberOfVariables; i+=1)
        {
            ...
            if (x != xLast)
            {
                Glib_ClipWindow(x0, y0, x0 + Width, y0 + Height);
                Glib_Draw(x0 + PlotData[i].Oldx[i], PlotData[i].Oldy[i], x0 + (int) (x), yp);
         		Glib_Flush();
            }
            PlotData[i].Oldx[i] = x;
...
static float Plot_FindVariable(IosDefn_PlaybackDataPktRecord Pkt, unsigned int n)
{
    switch (n)
    {
        ...
        case 22: return Pkt.EngPkt.Engines[0].Epr;
    }
...
{
    printf("Plot_SaveData not supported\n");
}
...
{
    printf("Plot_SetRecordingMode not supported\n");
}
...
{
    printf("SavePlotFile not supported\n");
}

/* --------------------------------------------- */
void Plot_SetPlayback(unsigned int Button)
{
    printf("Plot_SetPlayback not supported\n");
}

/* --------------------------------------------- */
void Plot_CheckPlayback(void)
{
    printf("Plot_CheckPlayback not supported\n");
}
